...
<div class="container">
  <div class="header" align="center"><a href="<? echo $_SERVER['PHP_SELF'];?>"><img class="headers" src="name.gif" alt="SQL" name="logo" width="780" height="120" id="Insert_logo" style="background: #C0C0C0; display:block;" /></a>
...
//восстановление исходной таблицы
   ...
   {
	   $drop=mysql_query("DROP TABLE `persons`",$db);
	   $drop_db=mysql_query("DROP DATABASE `sql`",$db);
	   $create_db=mysql_query("CREATE DATABASE `sql`",$db);
	   mysql_select_db("sql",$db);
	   $make=mysql_query("CREATE TABLE `sql`.`persons` (
...
PRIMARY KEY (`id`)
	   ...
	   if($insert)   
	   		echo"<br><p><center><b>Исходная база успешно восстановлена!</b></center></p>";
	   else
...
for ($j=0; $j<6; $j++) {
$type  = mysql_field_type($execute, $j);
$name  = mysql_field_name($execute, $j);
$len   = mysql_field_len($execute, $j);
$flags = mysql_field_flags($execute, $j);
echo "$name<BR>\n";
}*/
		...
		$ki++;
			...
			echo "<p align='center' style='color:#C3C3C3'><i>Таблиц в базе "; 
			t_quant("sql"); echo ": <b>"; 
			t_name("sql"); 
			echo "</b></i></p>";
...
//
	...
	{
		echo "<br><br><p><center>Строка запроса пуста! Вернитесь и введите в неё SQL запрос!</p></center>";	
	}
