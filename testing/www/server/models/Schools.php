...
result, date, time_min FROM school_results WHERE pupil_id=(SELECT id FROM pupils WHERE fio=$fio AND email=$email LIMIT 1)");
	...
	public function getAllSchoolsTests()
	{
		...
		$select_tests=$db->prepare("SELECT id, school_id, (SELECT name FROM school_classes WHERE id=class_id)AS class, (SELECT name FROM school_lessons WHERE id=lesson_id)AS lesson, name, description, time_min, rating, date, quantity, results, count FROM school_test_name");	
