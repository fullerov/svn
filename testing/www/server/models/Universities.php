...
class Universities
{
	...
	public function deleteMyUniver($univer_id, $user_id)
	{
		...
		if($delete->errorCode()==00000 and $del_cr->errorCode()==00000 and $del_fc->errorCode()==00000 and $del_sp->errorCode()==00000 and $del_rs->errorCode()==00000)
			 return true;
		else return false;
	...
	public function getTestResult($user_id)
	{
		...
		$results="SELECT id, test_id, student_id, univer_id, specialty_id, course_id, group_id, faculty_id, result, date, time_min FROM university_results WHERE ";
	...
	//метод возвращает информацию о университетах, группах, курсах, факультетах, специальности, студентах и названиях тестах пользователя
	public function getStudTestInfo($user_id)
	{
		$db=$this->db();
		
		$res['students']=self::getMyStudents($user_id);
		$res['univers']=self::getUniverIdName($user_id);
		$res['tests']=self::getTestIdName($user_id);
		$res['spec']=self::getSpecIdName($user_id);
		$res['course']=self::getCourseIdName($user_id);
		$res['fac']=self::getFacIdName($user_id);
		$res['group']=self::getGroupIdName($user_id);
		
		if($res['group']!=false and $res['fac']!=false and $res['course']!=false and $res['spec']!=false and $res['tests']!=false and $res['univers']!=false and $res['students']!=false)
			return $res;
		else return false;
		
	}
	
	//метод возвращает идентифекатор и имя всех тестов пользователя
	private function getTestIdName($user_id)
	{
		$db=$this->db();
		$user_id=$db->quote($user_id);
		
		$select=$db->prepare("SELECT id, name FROM university_test_name WHERE user_id=$user_id");
		$select->execute();
	...
	//метод возвращает все факультеты пользователя
	private function getFacIdName($user_id)
	{		
		$db=$this->db();
		$user_id=$db->quote($user_id);
		$select=$db->prepare("SELECT id, name FROM faculties WHERE univer_id=(SELECT id FROM universities WHERE user_id=$user_id)");
		$select->execute();
		
		if($select->errorCode()!=00000)
			return false;
		else return $select->fetchAll(PDO::FETCH_ASSOC);
	}
	
	//метод возвращает идентификатор специальности и её название по передаваемому параметру идентификатору пользователя
	private function getSpecIdName($user_id)
	{
		$db=$this->db();

		$facs=self::getFacIdName($user_id);
		$count=count($facs);
		$num=1;
		
		$specs="SELECT id, name FROM univer_specialty WHERE ";
		
		foreach($facs as $fac)
		{
			if($num!=$count)
				$specs.="faculty_id='".$fac['id']."' OR ";
			else 
				$specs.="faculty_id='".$fac['id']."';";
			
			$num++;
		}
		
		$sel_specs=$db->prepare($specs);
		$sel_specs->execute();
		
		if($sel_specs->errorCode()==00000)
			return $sel_specs->fetchAll(PDO::FETCH_ASSOC);
		else return false;
	}
	
	//метод возвращает идентификатор курса и его номер по передаваемому параметру идентификатору пользователя
	private function getCourseIdName($user_id)
	{
		$db=$this->db();
		...
		$courses="SELECT id, course FROM university_courses WHERE ";
		
		foreach($facs as $fac)
		{
			if($num!=$count)
				$courses.="faculty_id='".$fac['id']."' OR ";
			else 
				$courses.="faculty_id='".$fac['id']."';";
			
			$num++;
		}
		
		$sel_crss=$db->prepare($courses);
		$sel_crss->execute();
		
		if($sel_crss->errorCode()==00000)
			return $sel_crss->fetchAll(PDO::FETCH_ASSOC);
		else return false;
	}
	
	//метод возвращает группы пользователя по его идентификатору
	private function getGroupIdName($user_id)
	{
		$db=$this->db();
		$courses=self::getCourseIdName($user_id);
		
		$select="SELECT id, name FROM university_groups WHERE ";
		...
		{
			if($count!=$num)
				$select.="course_id='".$course['id']."' OR ";
			else
				$select.="course_id='".$course['id']."';";
			$num++;
		}
		
		$groups=$db->prepare($select);
		$groups->execute();
