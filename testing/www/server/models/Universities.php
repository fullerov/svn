...
(SELECT name FROM university_groups WHERE id=group_id) AS group_name, result, date, time_min FROM university_results WHERE student_id=(SELECT id FROM students WHERE fio=$fio AND email=$email LIMIT 1)");
	...
	public function getAllUniverTests()
	{
		...
		$select_tests=$db->prepare("SELECT id, university_id, (SELECT name FROM faculties WHERE id=faculty_id) AS faculty, (SELECT name FROM univer_specialty WHERE id=specialty_id) AS spec, (SELECT course FROM university_courses WHERE id=course_id)AS course, (SELECT name FROM university_groups WHERE id=group_id) AS groups, (SELECT name FROM university_lessons WHERE id=lesson_id)AS lesson, name, description, time_min, rating, date, quantity, results, count FROM university_test_name");
