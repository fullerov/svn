...
class Organizations
{
	...
	public function getAllOrgTests()
	{
		...
		$select_tests=$db->prepare("SELECT id, (SELECT themes FROM org_themes WHERE id=theme_id) AS theme, org_id, name, description, time_min, rating, date, quantity, results, count FROM org_test_name");	
		$select_orgs->execute();
