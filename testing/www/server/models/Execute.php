...
class Execute
{
	...
	public static function getUniverTests($first,$last)
	{
	    ...
	    ++$last;
		$db=self::db();
		$select=$db->prepare("SELECT `id`, `user_id`, `country_id`, `city_id`, `university_id`, `faculty_id`, `specialty_id`, `course_id`, `group_id`, `lesson_id`, `name`, `description`, `time_min`, `rating`, `date`, `quantity`, `results`, `count` FROM university_test_name WHERE id > $first AND id < $last");
		$select->execute();
		if($select->rowCount()==0)
			return false;
		//проверка на корректность исполнения запроса
	...
	public static function getSchoolTests($first, $last)
	{
	    ...
	    ++$last;
		$db=self::db();
		$select=$db->prepare("SELECT `id`, `user_id`, `country_id`, `city_id`, `school_id`, `class_id`, `lesson_id`, `name`, `description`, `time_min`, `rating`, `date`, `quantity`, `results`, `count` FROM school_test_name WHERE id > $first AND id < $last");
		$select->execute();
		if($select->rowCount()==0)
		{return false;}
		//проверка на корректность исполнения запроса
