...
class Execute
{
	...
	public static function getUserTests($first, $last)
	{
	    ...
	    ++$last;
		$db=self::db();
		$select=$db->prepare("SELECT `id`, `user_id`, `country_id`, `city_id`, `theme_id`, `name`, `description`, `time_min`, `rating`, `date`, `quantity`, `results`, `count`, `submits` FROM user_test_name WHERE id > $first AND id < $last");
		$select->execute();
		if($select->rowCount()==0)
		{return false;}
	...
	public static function getOrgTests($first, $last)
	{
	    ...
	    ++$last;
		$db=self::db();
		$select=$db->prepare("SELECT `id`, `user_id`, `country_id`, `city_id`, `theme_id`, `org_id`, `name`, `description`, `time_min`, `rating`, `date`, `quantity`, `results`, `count`, `submits` FROM org_test_name WHERE id > $first AND id < $last");
		$select->execute();
		if($select->rowCount()==0)
		{return false;}
		//проверка на корректность исполнения запроса
	...
	public static function getUniverTests($first,$last)
	{
	    ...
	    ++$last;
		$db=self::db();
		$select=$db->prepare("SELECT `id`, `user_id`, `country_id`, `city_id`, `university_id`, `faculty_id`, `specialty_id`, `course_id`, `group_id`, `lesson_id`, `name`, `description`, `time_min`, `rating`, `date`, `quantity`, `results`, `count`, `submits` FROM university_test_name WHERE id > $first AND id < $last");
		$select->execute();
		if($select->rowCount()==0)
			return false;
		//проверка на корректность исполнения запроса
	...
	public static function getSchoolTests($first, $last)
	{
	    ...
	    ++$last;
		$db=self::db();
		$select=$db->prepare("SELECT `id`, `user_id`, `country_id`, `city_id`, `school_id`, `class_id`, `lesson_id`, `name`, `description`, `time_min`, `rating`, `date`, `quantity`, `results`, `count`, `submits` FROM school_test_name WHERE id > $first AND id < $last");
		$select->execute();
		if($select->rowCount()==0)
		{return false;}
		//проверка на корректность исполнения запроса
