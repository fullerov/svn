...
class AuthorizationController implements IController
{
	...
	function edittestAction()
	{
		...
		if($params['addquestion']=='new' and isset($_POST['question']) and isset($_POST['answer']) and isset($_POST['add_question']) and isset($_POST['count']))
		{
			...
			if(!isset($empty))
			{
				...
				($add)? $view->content='Вопрос успешно добавлен!' : $view->content='Аналогичный тест уже есть в базе данных! '; 
	...
	function editorgtestAction()
	{
		...
		if($params['addquestion']=='new' and isset($_POST['question']) and isset($_POST['answer']) and isset($_POST['add_question']) and isset($_POST['count']))
		{
			...
			if(!isset($empty))
			{
				...
				($add)? $view->content='Вопрос успешно добавлен!' : $view->content='Аналогичный вопрос уже есть в базе данных! '; 
	...
	function resultsAction()
	{
		...
		$view->studtestinfo=Universities::get()->getStudTestInfo($_SESSION['loginfo']['id']);
