...
function getForm($type,$content)
{
	switch($type)
	{
		...
		case '2': return userForm($content); break; //студент +
		case '3': return userForm($content); break; //школьник +
		case '4': return teacherForm($content); break; //учитель -
		case '5': return userForm($content); break; //рабочий +
		case '6': return chiefForm($content); break; //начальник -
...
function userForm(array $content)
{
	...
	<i>Измененения вступят в силу после нажатия кнопки "Сохранить", для того чтобы удалить тест, нажмиет на кнопку "Удалить" в соответствующем поле столбца "Действие", а для редактирования вопросов теста нажмите на кнопку с количеством вопросов в столбце "Вопросы"</i><br><br>';
		...
		return 'Вы не создали ни одного теста!<br><span id="add_test_error"><a href="/create"><< К форме создания тестов</a></span>';
}
