...
{
printf('<h1>%s</h1><img title="%s" src="%s"><p>%s</p><span id="art_about_left">Добавил <a href="mailto:%s">%s</a> %s</span><span id="art_about_right">Оценка %s голосов %s</span>',$this->content['title'],$this->content['title'],$this->content['img'],$this->content['text'],Registration::getUserEmail($this->content['user_id']),Registration::getUserLogin($this->content['user_id']),$this->content['date'],$this->content['rating'],$this->content['votes']);
echo showComments($this->content['id'], $this->comments);
}
