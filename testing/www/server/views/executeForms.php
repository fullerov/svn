...
function getOrgTests($tests)
	{
		...
		if(is_array($tests))
		{
			foreach($tests as $test)
			{
				...
				<p>'.$test['description'].'<p>
				<br><h5>Для организации: '.Organizations::get()->getOrgLinkByTestid($test['id']).' Тема теста: <a href="search/orgtest/id/'.$test['theme_id'].'">'.Execute::getOrgTestTheme($test['theme_id']).'</a> Тест добавил: <a href="mailto:'.Registration::getUserEmail($test['user_id']).'">'.Registration::getUserLogin($test['user_id']).'</a><h6>Дата добавления: '.$test['date'].'</h6></h5><br>';
